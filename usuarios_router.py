...
class UsuarioResponse(BaseModel):
    id_user: str          # corregido: era int, es uuid
    name_user: str
    email_user: str
...
async def get_usuario(id_user: str):
    try:
        supabase = get_supabase_client()
        ...
        response = supabase.table("usuarios").select("""
            id_user, name_user, email_user, matricula_user, id_rol,
            rol!inner(name_rol)
        """).eq("id_user", id_user).single().execute()
...
async def update_usuario(id_user: str, data: UsuarioUpdate):
    try:
        supabase = get_supabase_client()

        check = supabase.table("usuarios").select("id_user").eq("id_user", id_user).execute()
        if not check.data:
            raise HTTPException(status_code=404, detail="Usuario no encontrado")
...
async def delete_usuario(id_user: str):
    try:
        supabase = get_supabase_client()
        ...
        # Si es profesor, eliminar también de tabla profesor
        if usuario["id_rol"] == 3:
...
async def toggle_rol(id_user: str):
    try:
        supabase = get_supabase_client()
